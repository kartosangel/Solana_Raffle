...
export async function getRafflerFromSlug(program: anchor.Program<Raffle>, slug: string) {
  const rafflers = await program.account.raffler.all([
    {
      memcmp: {
        offset: 8 + 32 + 4,
...
export async function getStakerFromSlug(program: anchor.Program<Stake>, slug: string) {
